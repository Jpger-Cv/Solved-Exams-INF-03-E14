...
<body>
    ...
    <main>
        ...
        <section class="glowny">
            ...
            <?php
                $result = mysqli_query($connect, "SELECT DISTINCT swinki.data_ur, swinki.miot, rasy.rasa FROM swinki JOIN rasy ON swinki.rasy_id = rasy.id WHERE swinki.rasy_id=1; ");    
                while($row = mysqli_fetch_array($result)){
            ...
            <?php
                $result = mysqli_query($connect, "SELECT imie, cena, opis FROM swinki WHERE rasy_id = 1;");
                while ($row = mysqli_fetch_array($result)) {
